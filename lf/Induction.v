...
(* [destruct] does not do induction... *)
*)
...
intros.
rewrite plus_comm.
assert (p + n = n + p). { rewrite plus_comm. reflexivity. }
rewrite <- H.
rewrite plus_assoc.
reflexivity.
Qed.
...
- rewrite <- mult_n_Sm. rewrite <- mult_n_Sm. rewrite <- mult_n_Sm.
  ...
  assert (n * p + m * p + n = n * p + n + m * p).
  { rewrite <- plus_assoc. rewrite <- plus_assoc.
    assert (m * p + n = n + m * p). { rewrite plus_comm. reflexivity. }
...
Theorem bin_to_nat_pres_incr : forall n, bin_to_nat (incr n) = S (bin_to_nat n).
Proof.
induction n.
- reflexivity.
- reflexivity.
- simpl. rewrite IHn. rewrite <- plus_n_Sm.
  assert (S (bin_to_nat n) + bin_to_nat n = S (bin_to_nat n + bin_to_nat n)).
...
(* [A] between rightmost [B] and [Z] are redundant. *)
...
Lemma double_bin_incr: forall n, double_bin (incr n) = incr (incr (double_bin n)).
Proof.
destruct n.
- reflexivity.
- reflexivity.
...
Lemma incr_double_bin: forall n, incr (double_bin n) = B n.
Proof.
destruct n.
- reflexivity.
- reflexivity.
...
induction n.
- reflexivity.
- simpl. rewrite <- double_plus. rewrite nat_to_bin_double. rewrite IHn. reflexivity.
- simpl. rewrite <- double_plus. rewrite nat_to_bin_double. rewrite IHn.
